...
const API = axios.create({ baseURL: 'http://localhost:5000' }) // Change to deployed backend-url
...
export const fetchPlayersBySearch = (searchQuery) => API.get(`/players/search?searchQuery=${searchQuery.search || 'none'}&tags=${searchQuery.tags}`)
export const createPlayer = (newPlayer) => API.post('/players', newPlayer)
export const likePlayer = (id) => API.patch(`/players/${id}/likePlayer`)
export const comment = (value, id) => API.post(`/players/${id}/commentPlayer`, { value })
